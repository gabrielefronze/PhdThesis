...
After that, the LHC will stop for two years (long shutdown), during which an upgrade that will allow to increase the luminosity reach in Pb-Pb from 8 to 50 kHz will be performed.
At the same time, ALICE will undergo a major upgrade of the electronics and acquisition that will allow to process the Pb-Pb data at 50 kHz and the pp data up tp 200 kHz.
...
The physics topics addressed by ALICE are the studies of physics probes characterized by very small signal-to-background ratios.
...
The upgrade of the electronics will allow to increase the data throughput of about two orders of magnitude larger than the one that ALICE experienced during LHC RUN1.
...
For example some detectors will be equipped with a zero suppression algorithm to reduce the volume of data without loosing useful information.
...
This characteristic hides part of the complexity deriving from different hardware choices and specializations of machines devoted to particular tasks, in fact the framework layer allows to make use of multi-core processors (CPUs) in a way almost transparent to the programmer.
Due to fundamental differences between GPU and FPGA models, the code which is intended to be executed on such accelerators will still require some higly customized code.
...
The introduction of such MPI-like process-based framework allows for dynamic spawning of workers, which is baked up with a message passing interface capable of using system buses and network interfaces seamlessly, is the backbone of the new infrastructure.
...
A requirement of a total integrated luminosity of $13\ nb^{-1}$ in Pb-Pb collisions was formulated with the ALICE Letter Of Intent (LOI) \ref{}, split in $10\ nb^{-1}$ at nominal solenoid magnetic field and $3\ nb^{-1}$ at a reduced field of $0.2\ T$.
...
\includegraphics[width=0.7\linewidth]{Chapters/O2/Figs/ALFA.png}
...
The communication layer is based on the ZeroMQ library and allows the passing of binary messages either across network interfaces or between threads within the same machine, by passing memory references.
...
During the LHC RUN2 ALICE will operate in continuous read-out mode.
...
\begin{itemize}
    ...
    \item the calibration data produced during a TF has a fixed size and constitutes a fixed overhead which has to be sent to the CDB. A longer TF would minimize the overhead contribution to data rate and the number of CDB accesses;
...
In the Run3, the MTR will lose its functionality of a triggering detector\footnote{although the possibility of providing a very basic trigger was kept in the electronics, mainly for UPC studies}, and it will only be used to provide parton identification. 
...
The addition of the MFT allows to improve the tracks resolution closer to the interaction point and possibly to resolve secondary vertexes.
...
Zero suppression and noisy/dead channels detection/suppression will be performed either by the FEE.
...
In a second case, the FEE allows to stimulate all of the strips to check if they're responsive.
...
This allows to detect the presence of dead channels.
...
The detection of noisy channels will allow to build a mask that will have to be transferred to the local boards. 
...
This will be most likely be done in the Condition DataBase (CDB).
Moreover, the detection of dead and noisy chamber will be the first step of the Quality Control (QC) of the detector, since it allows to pinpoint hot spots and dead zones of the RPC chambers.
...
\begin{itemize}
    \item The resolution of the algorithm has to be studied in four variables: 
    \begin{itemize}
    ...
    \item The muon reconstruction efficiency has to be evaluated;
...
In this case all the tracks are described three spatial coordinates and two slopes, one over $x$, the other over $y$.
...
Such measurement can be performed evaluating how many real muons are matched and correctly flagged.
