...
\section{These sections will be moved to top level introduction!}
...
\subsection{Glauber's model}\label{intro_glauber}
Glauber's model is the approximation usually used to model and analyze a nuclear collision, named after Roy Glauber.
...
\begin{figure}[!t]
\begin{center}
\includegraphics[width=0.45\linewidth]{Chapters/Analysis/Figs/woods-saxon.pdf}
...
\begin{figure}[!t]
\begin{center}
\includegraphics[width=0.45\linewidth]{Chapters/Analysis/Figs/glauber-side.pdf}
...
\begin{figure}[!t]
\begin{center}
\includegraphics[width=0.45\linewidth]{Chapters/Analysis/Figs/glauber-pbpb.pdf}
...
For ALICE the VZERO signal amplitude is the $N_{ch}$ estimation provider, hence the source of the main centrality evaluation, as shown in picture \ref{fig:GlauberVZERO}.


\begin{figure}[!t]
\begin{center}
\includegraphics[width=0.45\linewidth]{Chapters/Analysis/Figs/glauber-centrality.pdf}
...
\begin{figure}[!t]
\begin{center}
\includegraphics[width=0.45\linewidth]{Chapters/Analysis/Figs/glauber-vzero.pdf}
...
In order to study the modification of \upsi production in Pb-Pb collisions at $\sqrt{s_{_{\rm NN}}}=5.02$~\rm{TeV} the evaluation of $R_{\rm AA}$ should be performed through a modified expression with respect to the one presented in \ref{intro_glauber}:
\begin{equation} \label{eqn:raa}
R_{\rm{AA}} = \frac{N^{\Upsilon}}{ {\rm BR}_{\Upsilon \rightarrow \mu^{+} \mu^{-}} \cdot  (A\times\epsilon)_{\Upsilon \rightarrow \mu^{+} \mu^{-}} \cdot  N_{\mu\mu\text{-MB}} \cdot F_{\rm norm} \cdot \sigma_{\rm pp}^{\Upsilon} \cdot \langle T_{\rm AA} \rangle } ~~
\end{equation}
where:
\begin{itemize}
...
\item The normalization factor $N_{\mu\mu\text{-MB}} \cdot F_{\rm norm}$ is the product of the number of analyzed $\mu\mu\text{-MB}$ events, times the inverse of the probability to obtain an unlike-sign dimuon triggered event in a MB event~\cite{Adam:2016rdg};
\item $\sigma_{\rm pp}^{\Upsilon}$ is the reference \pp cross section and $\langle T_{\rm AA} \rangle$ represents the nuclear overlap function, which is evaluated in function of the collision centrality using the Glauber model~\cite{Abelev:2013qoq,Adam:2015ptt}.
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Experimental apparatus and data sample}

An extensive description of the ALICE apparatus can be found in the introduction of this thesis.
...
The V0 scintillators~\cite{Abbas:2013taa} provided the Minimum Bias (MB) trigger through the logical AND of the signals from the two arrays constitutes the Minimum Bias (MB) trigger.
The MB trigger is fully efficient for the studied 0--90\% most central collisions.
The V0 detector provides the centrality estimate through a fit of the raw signal via the Glauber model~\cite{Abelev:2013qoq,Adam:2015ptt}.
...
The muon tracker and the muon trigger are the detectors devoted to the identification and reconstruction of the produced muons, respectively.
The trigger condition used for data taking is a dimuon-Minimum Bias ($\mu\mu\text{-MB}$) trigger.
...
The adopted trigger is then formed by the logical AND of the MB trigger and an unlike-sign dimuon trigger with a \pt threshold of 1~GeV/$c$ for each of the two muons.
The former is provided by V0, the latter by the muon trigger.
In addition, in order to perform the event mixing procedure for background reduction, an equivalent trigger for like-sign dimuons has been used.

The data used for this analysis has been collected between October and November 2015, during the first \pbpb data taking campaign of ALICE within the  LHC RUN2.
...
In order to improve the purity of the dimuon sample a set of selection criteria has been applied on the muon tracks:
\begin{itemize}
\item Each track must have the transverse momentum $p_{\rm T} > 2$ GeV/$c$. This cut reduces the background from $K$ and $\pi$ decays without affecting the signal of \upsi decays. The contribution of this cut is marginal;
...
\item A cut on the distance of closest approach to the reconstructed interaction vertex of each track. This additional selection reduces the contribution from fake tracks and tracks originated by beam gas collisions. This cut has been tuned to be $6\times\sigma_{pDCA}$, where $\sigma_{pDCA}$ is the resolution of this quantity.
...
While the background can appear to be a side effect in much more abundant species, in the case of bottomonium the background needs to be treated with special attentions.
Two approaches are then possible to extract the yields from the fit of an invariant mass spectrum:
\begin{itemize}
\item The fit function can contain a phenomenological function that models the background shape. In this case the fit has to be performed on the raw spectrum;
\item Applying the event mixing method, the raw spectrum can be pre-processed, subtracting a properly scaled invariant mass spectrum which has previously been filled with dimuons composed using like-sign $\mu$ from the same collision and unlike-sign $\mu$ produced in different collisions. The rationale of this procedure is that filling a dimuon invariant mass spectrum with totally uncorrelated muons ($i.e.$ from different events) creates an invariant mass spectrum that represents the combinatorial background one should expect when filling the signal invariant mass spectrum.
\end{itemize}

The raw \upsi$(1S,2S,3S)$ yields are extracted using the sum of three extended Crystal Ball (CB) functions~\cite{ALICE-Quarkonia-signal-extraction}. 
The extended CB function consists of a Gaussian core with  power-law tails \ref{eqn:CB2}.
...
This correction is needed to re-normalize the \upsi yields to $4\pi$, namely the whole space.
...
\item Embedded MC simulations: the generator generates a \upsi decay that in then embedded in a real recorded event.  This allows to test the efficiency of detection of the simulated signal in a real event, with the full multiplicity observed in real data. This simulation can be used for efficiency studies.
\end{itemize}
...
A decrease of 2\% is observed in  $A\times\epsilon$ for the 0--10\% central collisions with respect to the 50--90\% sample due to a loss of detection efficiency in the muon trigger, caused by the higher occupancy in the most central events.
...
The reference cross section that has to be used to perform the nuclear modification factor computation has to be measured in $pp$ collisions at the same energy of the studied Pb-Pb ones.
ALICE data at $\sqrt{s}=5.02$ in $pp$ collisions was not sufficient to provide an estimate of the \upsi cross section.
...
The merged values for \pt, coming from LHCb data only, are reported in tables\ref{table:LHCbData276}\ref{table:LHCbData7}\ref{table:LHCbData8}.
The merged values for $y$, from both ALICE and LHCb, are reported in tables\ref{table:LHCbY1sData276}\ref{table:LHCbY1sData7}\ref{table:ALICEY1sData7}\ref{table:LHCbY1sData8}\ref{table:ALICEY1sData8}.
...
The plots representing the data points, the interpolation functions and the interpolated values are reported in figure \ref{fig:sigmapp}.
...
\caption{Interpolation of reference $\sigma_{pp}$ in \pt (top 4 graphs) and $y$ (bottom 4 graphs) bins. The measured points are reported with an error bar which represents both statistical and systematic uncertainties.}
\label{fig:sigmapp}
\end{center}
\end{figure}

The final cross section value is the average of the whole set of interpolated values.
...
The systematic uncertainty on the signal extraction has several contribution related to the various ingredients which constitute the nuclear modification definition in equation \ref{eqn:raa}.
...
The systematic uncertainty on the signal extraction is evaluated using various functions for the mass shape of the background contributions, as well as adopting two fitting ranges, $i.e.$ $(7-14)$ GeV/$c^2$ and $(7.5-14.5)$ GeV/$c^2$.
...
Due to lack of statistics for the higher mass bottomonium resonances, the mass position and width of the \upsiss and \upsisss has been fixed to the \upsis measured ones using the ration obtained from the PDG.
...
The values of $N^{\Upsilon}$ and their statistical uncertainties are obtained by taking the average of $N^{\Upsilon}$ and of the corresponding statistical uncertainties from the various fits.
...
In order to evaluate the systematic uncertainty due to this discrepancy two Monte Carlo productions have been performed.
one of them was performed using the default Monte Carlo efficiency parametrization of the muon trigger, while the second has been performed injecting a custom parametrization which included values measured during the data taking of the analyzed events.
...
This evaluation is performed via a procedure, detailed in ~\cite{Adam:2016rdg}, based on the redundancy of the tracking chamber information.
...
The systematic uncertainty is taken as the relative difference of the values obtained with the procedure based on MC and data.
...
The systematic uncertainties' magnitude affecting the $R_{\rm{AA}}$~s is comparable to those of the statistical ones.
...
A comparison between \upsis nuclear modification factor values at different energies might highlight a suppression (enhancement) dependence from the center of mass energy of the colliding system.
...
The ratio is compatible with unity within uncertainties, hence no energy hierarchy can be observed.
...
The centrality, $p_{\rm T}$ and $y$-dependences of the $\upsis$ $\raa$ at forward rapidity at $\snn=5.02$ \rm{TeV} are shown in Fig.\ref{fig:raa_data}. 
...
It is however possible to detect a non-significative hint of a correlation between $y$ and the suppression of $R_{\rm{AA}}$ which could possibly be addressed with further data taking campaigns.
...
The inclusive $\upsis$ $\raa$ measurements are compared in Fig.~\ref{fig:raa_models} to several theoretical models.
...
Transport models use a rate-equation approach which accounts for both suppression and (re)generation mechanisms in the QGP.
...
In model TM1 \cite{Du:2017qkv} the evolution of the thermal medium is based on a thermal-fireball expansion.
In TM1, feed-down measurements from ALICE and LHCb measurements \cite{Abelev:2014qha,Aaij:2014caa,Aaij:2014hla} are used.
TM1 predictions are shown as bands accounting for shadowing effects as calculated in ~\cite{Tuchin:2010pv}. 
...
TM1 does not take in account shadowing contributions.
...
The band of the model quantifies the heavy-quark potential uncertainty which has been estimated by including a $\pm 15\%$ variation of the Debye mass of the QCD medium which is tuned by a fit to the real-part of the lattice in-medium heavy-quark potential.
...
The nuclear modification factors of inclusive \upsis and \upsiss production at forward rapidity ($2.5<y<4$) and for $p_{\rm T}<15$ GeV/$c$ in \pbpb collisions at $\sqrt{s_{_{\rm NN}}}=5.02$~\rm{TeV} have been measured using the ALICE detector.

The computation of double ratios between \upsis and \upsiss $\raa$~s at $\sqrt{s_{_{\rm NN}}}=5.02$~\rm{TeV} and between \upsis $\raa$~s at $\sqrt{s_{_{\rm NN}}}=2.76$~\rm{TeV} and $\sqrt{s_{_{\rm NN}}}=5.02$~\rm{TeV} provided an hint of some additional features of the bottomonium evolution in QGP.
...
From the comparison with theoretical models no clear conclusion on the whether the recombination is present or not has been possible yet.
